...
<head>
    ...
    <title><?php echo $__env->yieldContent('admin_title', 'Admin Panel'); ?> – Accrosian CMS</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
            --bg: #0a0a0a; --surface: #111; --card: #1a1a1a; --border: rgba(255,255,255,0.08);
            --text: #f0f0f0; --text-light: #aaa; --muted: #666;
            --orange: #ff6b35; --orange-dark: #e55a25;
            --sidebar-w: 260px;
        ...
        /* Sidebar */
        .sidebar { width: var(--sidebar-w); background: var(--surface); border-right: 1px solid var(--border); height: 100vh; position: fixed; left: 0; top: 0; overflow-y: auto; display: flex; flex-direction: column; z-index: 100; }
        .sidebar-brand { padding: 24px 20px; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 12px; }
        .sidebar-brand img { height: 36px; border-radius: 8px; }
        .sidebar-brand-name { font-weight: 800; font-size: 1.1rem; }
        .sidebar-brand-name span { color: var(--orange); }
        .sidebar-brand-sub { font-size: 0.7rem; color: var(--text-light); margin-top: 2px; }
        .sidebar-nav { padding: 16px 12px; flex: 1; }
        .sidebar-section { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.1em; color: var(--muted); padding: 12px 8px 6px; }
        .sidebar-link { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 8px; color: var(--text-light); font-size: 0.9rem; transition: all 0.2s; margin-bottom: 2px; }
        .sidebar-link:hover, .sidebar-link.active { background: rgba(255,107,53,0.1); color: var(--orange); }
        .sidebar-link i { width: 18px; text-align: center; font-size: 0.85rem; }
        .sidebar-badge { margin-left: auto; background: var(--orange); color: #fff; font-size: 0.7rem; padding: 2px 7px; border-radius: 50px; font-weight: 700; }
        .sidebar-footer { padding: 16px 12px; border-top: 1px solid var(--border); }
        .sidebar-user { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 8px; background: var(--card); }
        .sidebar-avatar { width: 36px; height: 36px; border-radius: 50%; background: var(--orange); display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.9rem; flex-shrink: 0; }
        .sidebar-user-name { font-size: 0.85rem; font-weight: 600; }
        .sidebar-user-role { font-size: 0.75rem; color: var(--orange); }

        /* Main */
        .admin-main { margin-left: var(--sidebar-w); flex: 1; display: flex; flex-direction: column; min-height: 100vh; }
        .admin-topbar { background: var(--surface); border-bottom: 1px solid var(--border); padding: 16px 32px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 50; }
        .topbar-title { font-weight: 700; font-size: 1.1rem; }
        .topbar-actions { display: flex; align-items: center; gap: 16px; }
        .admin-content { padding: 32px; flex: 1; }

        /* Cards & UI */
        .admin-card { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 24px; margin-bottom: 24px; }
        .admin-card-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; }
        .admin-card-title { font-weight: 700; font-size: 1rem; }
        ...
        .admin-table th { text-align: left; padding: 12px 16px; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em; color: var(--text-light); border-bottom: 1px solid var(--border); }
        .admin-table td { padding: 14px 16px; border-bottom: 1px solid var(--border); font-size: 0.9rem; color: var(--text-light); vertical-align: middle; }
        .admin-table tr:hover td { background: rgba(255,255,255,0.02); }
        .admin-table tr:last-child td { border-bottom: none; }

        /* Buttons */
        .btn-admin { display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; border: none; transition: all 0.2s; text-decoration: none; }
        .btn-admin-primary { background: var(--orange); color: #fff; }
        .btn-admin-primary:hover { background: var(--orange-dark); }
        .btn-admin-outline { background: transparent; border: 1px solid var(--border); color: var(--text-light); }
        .btn-admin-outline:hover { border-color: var(--orange); color: var(--orange); }
        .btn-admin-danger { background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); color: #ef4444; }
        ...
        .badge-read { background: rgba(107,114,128,0.15); color: #9ca3af; }
        ...
        .badge-admin { background: rgba(255,107,53,0.15); color: var(--orange); }
        ...
        .toggle-slider { position: absolute; inset: 0; background: var(--border); border-radius: 50px; cursor: pointer; transition: 0.3s; }
        ...
        /* Page breadcrumb */
        .breadcrumb { display: flex; align-items: center; gap: 8px; font-size: 0.85rem; color: var(--text-light); margin-bottom: 24px; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<?php echo e(route('admin.contact.index')); ?>" class="sidebar-link <?php echo e(request()->routeIs('admin.contact*') ? 'active' : ''); ?>">
            <i class="fas fa-envelope"></i> Contact Forms
            <?php $newCount = \App\Models\ContactSubmission::where('status','new')->count(); ?>
            <?php if($newCount > 0): ?>
                <span class="sidebar-badge"><?php echo e($newCount); ?></span>
            <?php endif; ?>
        </a>
...
<div class="admin-main">
    <header class="admin-topbar">
        <div class="topbar-title"><?php echo $__env->yieldContent('admin_title', 'Dashboard'); ?></div>
        <div class="topbar-actions">
            <span style="color:var(--text-light);font-size:0.85rem">Welcome, <?php echo e(auth()->user()->name); ?></span>
            <a href="<?php echo e(route('home')); ?>" class="btn-admin btn-admin-outline btn-admin-sm" target="_blank">
...
<?php /**PATH C:\xamppNew\htdocs\accroNew\resources\views/layouts/admin.blade.php ENDPATH**/ ?>