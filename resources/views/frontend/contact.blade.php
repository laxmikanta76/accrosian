...
@section('meta_description', $page->meta_description ?? 'Get in touch with Accrosian for software development projects, consultations, and quotes.')
            ...
            <div class="contact-info">
                ...
                <div class="contact-info-card reveal reveal-delay-2">
                    ...
                    <div>
                        ...
                        <div class="contact-info-value"><a href="tel:{{ $setting->contact_phone }}">{{ $setting->contact_phone }}</a></div>
                    </div>
                </div>
                @endif
                ...
                <div class="contact-info-card reveal reveal-delay-3">
                    ...
                    <div>
                        ...
                        <div class="contact-info-value"><a href="mailto:{{ $setting->contact_email }}">{{ $setting->contact_email }}</a></div>
                    </div>
                </div>
                @endif
                ...
                @if(isset($setting) && ($setting->linkedin || $setting->twitter || $setting->instagram || $setting->facebook))
                <div class="reveal" style="margin-top:12px">
                    ...
                    <div class="social-links">
                        @if($setting->linkedin)<a href="{{ $setting->linkedin }}" class="social-link" title="LinkedIn" target="_blank"><i class="fab fa-linkedin-in"></i></a>@endif
                        @if($setting->twitter)<a href="{{ $setting->twitter }}" class="social-link" title="Twitter" target="_blank"><i class="fab fa-x-twitter"></i></a>@endif
                        @if($setting->instagram)<a href="{{ $setting->instagram }}" class="social-link" title="Instagram" target="_blank"><i class="fab fa-instagram"></i></a>@endif
                        @if($setting->facebook)<a href="{{ $setting->facebook }}" class="social-link" title="Facebook" target="_blank"><i class="fab fa-facebook-f"></i></a>@endif
                    </div>
                </div>
                @endif
            </div>
                ...
                <form action="{{ route('contact.store') }}" method="POST" id="contact-form">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" id="email" name="email" placeholder="john@company.com" value="{{ old('email') }}" required />
                        </div>
                        <div class="form-group">
                            <label for="phone">Phone Number</label>
                            <input type="tel" id="phone" name="phone" placeholder="+91 98XXXXXXXX" value="{{ old('phone') }}" />
                        </div>
                        <div class="form-group">
                            <label for="subject">Subject</label>
                            <select id="subject" name="subject">
                                <option value="">Choose a Service</option>
                                @foreach(['Web Development','Mobile App Development','Cloud Solutions','UI/UX Design','AI & Machine Learning','Software Consulting','Multiple Services','General Enquiry'] as $opt)
                        ...
                        <div class="form-group full">
                            ...
                            <textarea id="message" name="message" placeholder="Describe your project, goals, timeline, and any specific requirements..." required>{{ old('message') }}</textarea>
                        </div>
                    </div>
