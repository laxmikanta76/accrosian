    ...
    <table class="admin-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><code
                        style="background:var(--surface);padding:2px 6px;border-radius:4px;font-size:0.8rem">{{ $service->slug }}</code>
                </td>
                <td><span
                        class="badge badge-{{ $service->status ? 'active' : 'inactive' }}">{{ $service->status ? 'Active' : 'Inactive' }}</span>
                </td>
            ...
            <tr>
                <td colspan="6" style="text-align:center;padding:40px;color:var(--text-light)">No services found. <a
                        href="{{ route('admin.services.create') }}" style="color:var(--orange)">Add one</a></td>
            </tr>
            @endforelse
        </tbody>
    </table>
